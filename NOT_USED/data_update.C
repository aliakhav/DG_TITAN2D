      ...
      else//-- sender's generation is diffrent from receptor's
	{
	  ...
	  if(recv->sender_gen<EmTemp->get_gen())//--sender is bigger than receptor
	      ...
	      if(recv->sender_refined)
		{
		  ...
		  Element* father1 = (Element*)(HT_Elem_Ptr->lookup(EmTemp->getfather()));
		  unsigned* father_nodes = father1->getNode();
		  Node* father_mid = (Node*)(HT_Node_Ptr->lookup(father_nodes+(start+4)*KEYLENGTH));
		  
		  keyP = EmTemp->getNode();
		  Node* NdTemp = (Node*)(HT_Node_Ptr->lookup(keyP+start*KEYLENGTH));
		  
		  if(*(NdTemp->pass_key())== *(father_mid->pass_key()))//--KEYLENGTH
		    {
		  ...
		  else
		    {
		      ...
		      assert(*(NdTemp->pass_key())== *(father_mid->pass_key()));//-- debugging port
