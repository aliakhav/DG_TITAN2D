...
extern void  depchk(Element*, HashTable*, int*, Element*[], int*);
...
void  H_adapt(HashTable* HT_Elem_Ptr, HashTable* HT_Node_Ptr, int h_count, 
	      double target, MatProps* matprops_ptr, double* maxFluxIntegral)
  /*----------------------------------------------------------------------*
  ...
  int debug_ref_flag = 0;
   for(i=0; i<hash_size; i++)//-- every process begin to scan their own hashtable
	...
	{	  
	  ...
	  if(EmTemp->get_new_old()==OLD && EmTemp->get_refined_flag() == 0)
	    {
#ifdef DEBUG	   
	    printf("lb_weght %e",EmTemp->get_lb_weight());
	    assert(EmTemp->get_lb_weight()< 1000000000000000);
#endif

	      refine_flag = 0;
	      if(*(EmTemp->get_el_error()) > target+WEIGHT_ADJUSTER &&
                  EmTemp->get_gen() < REFINE_LEVEL)
		refine_flag = 1;
	      else if(EmTemp->get_gen() < REFINE_LEVEL) {  
		/* check to see if my pile height is below a threshold value and my 
		   neighbor's pile height is above the threshold -- if so, refine me */
		refine_flag = 0;
		if(*(EmTemp->get_el_solution()) < REFINE_THRESHOLD) {
		  Element* elmneigh;
		  int j=0;
		  int* neigh_proc = EmTemp->get_neigh_proc();
		  while(j<8 && refine_flag == 0) {
		    if(neigh_proc[j] >= 0 && myid==neigh_proc[j]) {
		      elmneigh = (Element*) HT_Elem_Ptr->lookup(EmTemp->get_neighbors()+j*KEYLENGTH);
		      assert(elmneigh);
		      if(*(elmneigh->get_el_solution()) >= REFINE_THRESHOLD){
			refine_flag = 1;
		      }
		    }
		    j++;
		...
		if (EmTemp->get_fluxIntegral() > 0.75*(*maxFluxIntegral) && 
		    *(EmTemp->get_el_solution()) > 2*GEOFLOW_TINY
		    && EmTemp->get_gen() < REFINE_LEVEL )
		  refine_flag=1;		
	      } 

	      //   printf("max flux integral %e flux int %e \n",maxFluxIntegral,EmTemp->get_fluxIntegral());
  
	      //  double* d_temp = EmTemp->get_el_solution();
	      //  double pile = 0;
	      //  for(j=0;j<4;j++)
	      //  pile += d_temp[j*EQUATIONS]*d_temp[j*EQUATIONS];
	      // if(sqrt(pile) > GEOFLOW_TINY && EmTemp->get_gen() < REFINE_LEVEL)
	      //	refine_flag = 1;
	      //    else  
	      //refine_flag = 0;
	       
	      //if(*(EmTemp->get_elm_loc()) == 4 && 
	      //    	 *(EmTemp->get_elm_loc()+1) == 9){
	      //   	refine_flag = 1;
	      //  	printf("refine flag %i \n",refine_flag);
	      //   }
	      //  else 
	      // 	refine_flag = 0;
  ...
  for(i=0; i<hash_size; i++)
    {
      ...
      while(entryp)
        {
          Element* Curr_El=(Element*)(entryp->value);
          entryp=entryp->next;  //get next element before deleting this one...
          int refined = Curr_El->get_refined_flag();
          if(refined != 0)//if this is a refined element don't involve!!!
            {
              // deleting the father
              HT_Elem_Ptr->remove(Curr_El->pass_key(), 1);
              delete Curr_El;       
            }
