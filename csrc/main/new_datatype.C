...
{
  /* Create new MPI datatype: elemtype definition in struct.h
...
/*  MPI_Datatype ELEMTYPE;
  ...
  int           blockcounts[3]={36, 25*KEYLENGTH, 28+EQUATIONS+2*ELM_DOF};
  ...
  MPI_Address(&(elem->node_elevation[0]), &displs[2]);  
  ...
  int           blockcounts4[2]={1, 2*KEYLENGTH};
  MPI_Datatype  types4[2]={MPI_INT, MPI_UNSIGNED};
  MPI_Aint      displs4[2]={0,0};
  
  enriched_neighbor_pack* enrico=new enriched_neighbor_pack;
  
  MPI_Address(&(enrico->order), &displs4[0]);
  MPI_Address(&(enrico->target_element), &displs4[1]);
  
  for(d=1; d>=0; d--)
    displs4[d]-=displs4[0];


  MPI_Type_struct(2, blockcounts4, displs4, types4, &ENRICHED_INFO);
  MPI_Type_commit(&ENRICHED_INFO);
