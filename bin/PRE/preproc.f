...
c     topo data
      ...
      double precision oldxmin, oldymin, oldxmax, oldymax
      ...
      xlength = .90d0*(oldxmax - oldxmin)
      ylength = .90d0*(oldymax - oldymin)

      xmin = xmin + .05d0*(oldxmax - oldxmin)
      xmax = xmax - .05d0*(oldxmax - oldxmin)
      write(*,*) xmin,xmax

      xlength = xmax - xmin
      ymin = ymin + .05d0*(oldymax - oldymin)
      ymax = ymax - .05d0*(oldymax - oldymin)
      write(*,*) ymin,ymax

      ylength = ymax - ymin

      nx = ny*(.00001d0+xlength/ylength)
      if(nx .le.  0) then
         nx = 10
